...
async fn list(client: &ApiClient, status_filter: Option<String>) -> Result<()> {
    ...
    let rows: Vec<Row> = endpoints
        ...
        .map(|ep| Row {
            ...
            hostname: ep["network"]["primary_hostname"]
                .as_str()
                .unwrap_or("-")
                .to_string(),
            ip: ep["network"]["primary_ip"]
                .as_str()
                .unwrap_or("-")
                .to_string(),
...
async fn get(client: &ApiClient, id: &str) -> Result<()> {
    ...
    if client.output_format == "json" {
        ...
        println!(
            "Hostname: {}",
            endpoint["network"]["primary_hostname"]
                .as_str()
                .unwrap_or("-")
        );
        println!(
            "IP:       {}",
            endpoint["network"]["primary_ip"].as_str().unwrap_or("-")
        );
        println!(
            "OS:       {}",
            endpoint["identity"]["distro"].as_str().unwrap_or("-")
        );
        println!(
            "Arch:     {}",
            endpoint["identity"]["arch"].as_str().unwrap_or("-")
        );
    }
    Ok(())
}
